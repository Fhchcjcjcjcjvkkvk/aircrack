...
# Track sequence numbers and lost packets with a time-based sliding window
packet_tracking = defaultdict(lambda: {'last_seq': None, 'lost_count': 0, 'timestamps': []})
...
# Function to sniff packets and capture sequence numbers
def packet_callback(packet):
    """Process sniffed packets and track lost packets for each SSID."""
...
def sniff_packets():
    """Start sniffing Wi-Fi packets to track sequence number loss."""
...
def scan_wifi():
    ...
    while True:
        ...
        clear_console()
        print(f"{'BSSID':<20} {'SSID':<30} {'Signal (dBm)':<15} {'Auth':<20} {'Lost Packets':<25}")
        print("=" * 110)
        ...
        for network in networks:
            ...
            # Track lost packets (this will work in conjunction with sniffing)
            lost_count = packet_tracking[ssid]['lost_count']

            print(f"{bssid:<20} {ssid:<30} {signal:<15} {auth:<20} {lost_count:<25}")
